<template>
  <div id="contactUs" class="components-container">
    ...
    <div style="width:100%;overflow:hidden;">
        <el-table :data="contactUss" :v-loading="loading"
  ...
  export default {
    ...
    methods: {
      ...
      getData(){
        ...
        getAboutUS({name:'contactUs',page:this.filtr.page, pageSize:this.filtr.pageSize}).then(res => {
          console.log(res.data);
          if (!res.data.code) {
            that.page.total = res.data.total;
            that.contactUss = res.data.aboutUSList;
            for(let i = 0; i < that.contactUss.length; i++){
                that.contactUss[i].createDate =  dateFormat(that.contactUss[i].createDate);
                that.contactUss[i].lastEditDate =  dateFormat(that.contactUss[i].lastEditDate);
                that.contactUss[i].content = that.contactUss[i].content.replace(/<[^>]+>/g,"");  
            }
            console.log(res.data);
          } else {
              this.$message({
      ...
      handleDetail(index, row) {
        this.$router.push({name:'contactUsDetail',params:{id:row._id}}) 
      },
      ...
      handleAdd(){
          this.$router.push({name:'contactUsEdit'}) 
      },
      handleUpdate(index,row){
          this.$router.push({name:'contactUsEdit',params:{id:row._id}})
      },
      ...
      handleApply(index, row) {
          ...
          this.$confirm('该文件将应用于留学生网站', '提示', {
                ...
                that.loading = true;
                applyAboutUS({name:'contactUs', id: row._id }).then(res => {
                    this.$message({
                        type: 'success',
                        message: res.data.message
                    });
