          ...
          <el-form :inline="true"
                ...
                <el-form-item label="关键字">
                    <el-input v-model="formInline.keywords"
                              placeholder="标题/发布人/最后修改人"></el-input>
                </el-form-item>
        ...
        <div class="feedback__container">
            <div class="feedback__table">
                <el-table :data="newsList"
                    ...
                    <el-table-column  min-width="500" label="新闻">
...
<script>
import { getNewsList, removeNews, download , exportToExel,batchDeleteNews } from 'api/api';
import 'common/index';
...
export default {
    ...
    methods: {
      getData(){
        ...
        this.loading = true;
        getNewsList({id:'',page:this.filtr.page, pageSize:this.filtr.pageSize, keywords: this.formInline.keywords}).then(res => {
          console.log(res.data);
          if (!res.data.code) {
            that.page.total = res.data.total;
            that.newsList = res.data.newsList;
            for(let i = 0; i < that.newsList.length; i++){
                that.newsList[i].createDate =  dateFormat(that.newsList[i].createDate);
      ...
      handleUpdate(index,row){
          this.$router.push({name:'newsEdit',params:{id:row._id}})
      },
      handleDelete(index, row) {
          ...
          this.$confirm('此操作将永久删除该条新闻, 是否继续?', '提示', {
                ...
                that.loading = true;
                removeNews({ id: row._id }).then(res => {
                    this.$message({
                        type: 'success',
                        message: '删除成功!'
                    });
      ...
      handleBatchDelete(){
          let that = this;
          this.$confirm('确定要删除所选新闻吗?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                batchDeleteNews({newsList:that.multipleSelection}).then(res =>{
                    if(!res.code){
                        that.$message({
                          type: 'success',
                          message: '删除成功'
